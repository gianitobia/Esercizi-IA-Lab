...
:- not M-2 { enroll(C) : hours(C,H) = H ,course(C,S,H)} M, max_hours(M).
:- #min { enroll(C) : hours(C,H) = H ,course(C,S,H)} 2.
:- 6 #max { enroll(C) : hours(C,H) = H ,course(C,S,H)}.

courses(N) :- N = #count { enroll(C) : course(C,_,_) }.
hours(N)   :- N = #sum { enroll(C) : hours(C,H) = H ,course(C,S,H)}.
