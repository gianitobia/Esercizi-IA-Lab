...
(deftemplate studente 
	...
	(slot nome)
	(slot residenza)
	(slot iscritto-laurea)
	(slot univ)
)
(deftemplate carico-didattico
	(slot matr)
	(slot corso)
)
...
(defrule r2
	...
	(indirizzo ?x RSI) => (assert 
		(corso-da-inserire ?x ReteDiElaboratori)
...
(defrule r3
	...
	(indirizzo ?x IC) => (assert 
		(corso-da-inserire ?x SistInt)
