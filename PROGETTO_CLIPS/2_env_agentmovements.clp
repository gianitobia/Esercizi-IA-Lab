...
;controlla che sia possibile effettuare una forward in avanti
...
;Notifica l'avvenuto scontro tra il robot e una persona (in avanti)
;essendo l'unica cosa che cambia nello scenario e poiche
;il robot a conoscenza a priori sull'ambiente
...
;controlla che sia possibile effettuare una forward in giu
(defrule forward-south-ok	(declare (salience 20))    
	?f2 <- (status (step ?i) (time ?t))  
	(exec (step ?i) (action  Forward))
...
(defrule forward-south-bump 	(declare (salience 20))    
	?f2 <- (status (step ?i) (time ?t)) 
	(exec (step ?i) (action  Forward))
	?f1 <- (agentstatus (step ?i) (pos-r ?r) (pos-c ?c)(direction south))
	(cell (pos-r =(- ?r 1)) (pos-c ?c) (contains ~Empty&~Parking))
